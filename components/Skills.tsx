...
function SkillColumn({
  ...
  return (
    <article className="skill-card readable-reveal group relative overflow-hidden rounded-[2.25rem] border border-pink-200/80 bg-white/80 p-7 shadow-[0_28px_80px_rgba(219,39,119,0.14)] backdrop-blur transition duration-300 hover:-translate-y-1 hover:border-pink-300 hover:shadow-[0_36px_110px_rgba(219,39,119,0.22)] md:p-8">
      ...
      <div className="relative">
        <h3 className="text-3xl font-black tracking-[-0.045em] text-[#24151d]">
          {title}
        </h3>

        <p className="mt-4 max-w-xl text-base font-medium leading-7 text-[#6f4a5d]">
          {subtitle}
        </p>
        ...
        <div className="mt-8 flex flex-wrap gap-3">
          {items.map((skill) => (
            <span
              key={skill}
              className="skill-pill rounded-full border border-pink-200 bg-pink-50/85 px-4 py-2 text-sm font-semibold text-[#6f4a5d] transition duration-300 hover:-translate-y-0.5 hover:border-pink-300 hover:bg-pink-100 hover:text-[#F72D9A]"
            >
              {skill}
            </span>
          ))}
